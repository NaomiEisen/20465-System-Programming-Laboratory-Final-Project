...
boolean handle_directive(ASTNode *node, CmpData *cmp_data) {
    ...
    /* Find corresponding command_index */
    dir_index = find_directive(node->operation);
    ...
    dir_command = directive_table[dir_index].dir_command;
    switch (dir_command) {
        case DATA:
    ...
    if (node->label != NULL){
        if (dir_command == EXTERN || dir_command == ENTRY) {
            ...
            if (validate_label(node, id_start, cmp_data) == FALSE) {
...
int find_command(const char *command) {
    ...
    while (command_table[i].command_str[0] != '\0') {
        if (strcmp(command_table[i].command_str, command) == 0) {
            return i;
        }
        i++;
    }
...
int find_directive(const char *command) {
    ...
    while (directive_table[i].command_str[0] != '\0') {
        if (strcmp(directive_table[i].command_str, command) == 0) {
            return i;
        }
        i++;
    }
